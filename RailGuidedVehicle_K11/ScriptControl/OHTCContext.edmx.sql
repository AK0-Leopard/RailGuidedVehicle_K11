...
GO
IF OBJECT_ID(N'[OHTC_DevModelStoreContainer].[VTRANSFER]', 'U') IS NOT NULL
    DROP TABLE [OHTC_DevModelStoreContainer].[VTRANSFER];
GO
...
CREATE TABLE [dbo].[VTRANSFER] (
    ...
    [RESULT_CODE] char(2)  NULL,
    [EXCUTE_CMD_ID] char(64)  NULL,
    [CARRIER_INSER_TIME] datetime  NULL,
...
-- Creating primary key on [SEC_ID] in table 'ASECTION_CONTROL_100'
...
-- Creating primary key on [SEC_ID] in table 'ASECTION'
ALTER TABLE [dbo].[ASECTION]
ADD CONSTRAINT [PK_ASECTION]
    PRIMARY KEY CLUSTERED ([SEC_ID] ASC);
GO

-- Creating primary key on [SEG_ID] in table 'ASEGMENT'
ALTER TABLE [dbo].[ASEGMENT]
ADD CONSTRAINT [PK_ASEGMENT]
    PRIMARY KEY CLUSTERED ([SEG_ID] ASC);
GO
...
-- Creating primary key on [EQPT_ID], [RPT_DATE_TIME], [ALAM_CODE] in table 'ALARM'
