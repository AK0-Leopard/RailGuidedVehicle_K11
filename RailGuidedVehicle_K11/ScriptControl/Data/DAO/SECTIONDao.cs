...
namespace com.mirle.ibg3k0.sc.Data.DAO
{
    public class SectionDao
    {
        ...
        public Dictionary<string, int> loadGroupBySecIDAndThroughTimes(DBConnection_EF con)
        {
            ...
            var query = from section in con.ASECTION
                        join cmd_detail in con.ACMD_DETAIL
        ...
        public List<ASECTION> loadByToAdrs(CommObjCacheManager cache, List<string> to_adrs)
        {
            if (to_adrs == null || to_adrs.Count == 0)
                return null;
            var query = from section in cache.getSections()
                            //where to_adrs.Contains(section.TO_ADR_ID.Trim())
                        where to_adrs.Any(to_adr => to_adr.Trim() == section.REAL_TO_ADR_ID.Trim())
                        select section;
            return query.ToList();
        }
