...
namespace com.mirle.ibg3k0.sc.Data.DAO
{
    public class BlockZoneQueueDao
    {
        ...
        public List<string> getCurrentBlockID(DBConnection_EF con, String vhID)
        {
            var query = from block in con.BLOCKZONEQUEUE
                        where block.CAR_ID == vhID.Trim()
                              && block.STATUS.CompareTo(SCAppConstants.BlockQueueState.Release) < 0
                        orderby block.REQ_TIME
                        select block.ENTRY_SEC_ID;
            return query.ToList();
        }

        public List<BLOCKZONEQUEUE> getAllCurrentBlockID(DBConnection_EF con)
        {
            var query = from block in con.BLOCKZONEQUEUE
