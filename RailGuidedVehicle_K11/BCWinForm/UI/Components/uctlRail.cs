...
namespace com.mirle.ibg3k0.bc.winform.UI.Components
{
    ...
    public class PointObject : ICloneable
    {
        ...
        public Dictionary<string, PointObject> BindPointByGroupRails { get; private set; }
        ...
        public void BindingPointByGroupRails(string sectionID, PointObject _pointObj)
        {
            if (!BindPointByGroupRails.ContainsKey(sectionID))
            {
                BindPointByGroupRails.Add(sectionID, _pointObj);
            }
        }
...
#pragma warning disable CS0414 // 已指派欄位 'GroupRails.vehicleInterval'，但從未使用過其值。
        private int vehicleInterval = 0;//mm
#pragma warning restore CS0414 // 已指派欄位 'GroupRails.vehicleInterval'，但從未使用過其值。
        ...
        private void buildSectionLable()
        {
            try
            {
                ...
                int loacation_x = 0;
                int loaction_y = 0;
                findLableLocation(lbl_width, lbl_height, ref loacation_x, ref loaction_y);
...
#pragma warning restore CS0168 // 已宣告變數 'ex'，但從未使用過它。
        ...
        public void refreshLabelLocation()
        {
            int lbl_width = 60;
            int lbl_height = 20;
            int loacation_x = 0;
            int loacation_y = 0;
            findLableLocation(lbl_width, lbl_height, ref loacation_x, ref loacation_y);

            lbl_SectionID.Location = new Point(loacation_x, loacation_y);
        }
        ...
        private void ReverseWalking(uctlNewVehicle vh, double distance_scale, uctlRail matchRail, int railInterval_Pix)
        {
            int Location_X = (int)matchRail.p_Points[1].RealPointf.X;
            int Location_Y = (int)matchRail.p_Points[1].RealPointf.Y;
