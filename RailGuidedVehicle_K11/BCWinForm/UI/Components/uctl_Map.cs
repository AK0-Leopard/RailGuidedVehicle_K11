...
namespace com.mirle.ibg3k0.bc.winform.UI.Components
{
    public partial class uctl_Map : UserControl
    {
        ...
        private void initialMapSpace()
        {
            ...
            else if (BCFUtility.isMatche(mainForm.BCApp.SCApplication.BC_ID, SCAppConstants.WorkVersion.VERSION_NAME_ASE_K11_DEMO))
            {
                space_Height_m = 35000; //AGVC 地圖地圖大小
                space_Width_m = 80000;
                zoon_Factor = 130;
            ...
            {
                space_Height_m = 13000; //AGVC 地圖地圖大小
                space_Width_m = 50000;
                zoon_Factor = 100;
                defaultMaxScale = 10;
            }
        ...
        private bool readNewVehicleDatas()
        {
            ...
            try
            {
                ...
                m_objItemPic = new PictureBox[iVhclCount * 3];
                ...
                for (int ii = 0; ii < iVhclCount; ii++)
                {
                    ...
                    PictureBox pic_CstIcon_r = new PictureBox();
                    pic_CstIcon_r.Size =
                        new Size(Resources.Action__Cassette_.Size.Width / 2, Resources.Action__Cassette_.Size.Height / 2);
                    pic_CstIcon_r.SizeMode = PictureBoxSizeMode.Zoom;
                    pic_CstIcon_r.BackColor = Color.Transparent;
                    pic_CstIcon_r.Visible = false;
                    pic_CstIcon_r.Image = Resources.Action__Cassette_;
                    PictureBox pic_CstIcon_l = new PictureBox();
                    pic_CstIcon_l.Size =
                        new Size(Resources.Action__Cassette_.Size.Width / 2, Resources.Action__Cassette_.Size.Height / 2);
                    pic_CstIcon_l.SizeMode = PictureBoxSizeMode.Zoom;
                    pic_CstIcon_l.BackColor = Color.Transparent;
                    pic_CstIcon_l.Visible = false;
                    ...
                    m_objItemPic[ii * 3] = pic_AlarmStatus;
                    m_objItemPic[(ii * 3) + 1] = pic_CstIcon_r;
                    m_objItemPic[(ii * 3) + 2] = pic_CstIcon_l;
        ...
        private bool BidingRailPointAndGroupForRail()
        {
            ...
            for (int i = 0; i < listGroup.Count - 1; i++)
            {
                if (!BCFUtility.isMatche(listGroup[i].SECTION_ID, listGroup[i + 1].SECTION_ID))
                    continue;
                findMatchRailPoint(listGroup[i].SECTION_ID, listGroup[i].RAIL_ID, listGroup[i + 1].RAIL_ID);
            }
            ...
            foreach (ASEGMENT seg in listSegment)
            {
                List<GroupRails> segSections = m_DicSectionGroupRails.Values
                    .Where(group => group.Segment_Num.Trim() == seg.SEG_ID.Trim()).ToList();
                m_DicSegmentGroupRails.Add(seg.SEG_ID.Trim(), segSections);
            }
            return (bRet);
        }
        ...
        private void RailBringToFrontBySectionGroup(string[] SelectSections)
        {
            ...
            {
                if (!m_DicSectionGroupRails.ContainsKey(section.Trim())) continue;
                foreach (var rail in m_DicSectionGroupRails[section.Trim()].uctlRails)
                {
                    Adapter.Invoke((obj) =>
                    {
                        pnl_Map.Controls.SetChildIndex(rail.Key, zInitialIndex + count++);
